...
def flow_phenotype(y: np.ndarray, e: int, c: int, s: int, x: int, delta: float) -> np.ndarray:
    ...
    # Matrix exponential for general B
...
class JumpIntensities:
    ...
    def cycle_switch_rate(self, cell: Cell, c_new: int, t: float) -> float:
        ...
        drug_mod = 1.0
        u = self.get_drug_conc(t).get("cell_cycle_inhibitor", 0.0)
    ...
    def sen_switch_rate(self, cell: Cell, s_new: int, t: float) -> float:
        ...
        k_effect = 1.0 + 0.01 * cell.total_ecdna()
    ...
    def all_switch_rates(self, cell: Cell, t: float) -> List[Tuple[str, dict, float]]:
        ...
        for c_new in cfg.CYCLE_STATES:
            if c_new != cell.c:
                r = self.cycle_switch_rate(cell, c_new, t)
                if r > 0:
        ...
        for s_new in cfg.SEN_STATES:
            if s_new != cell.s:
                r = self.sen_switch_rate(cell, s_new, t)
                if r > 0:
    ...
    def ecdna_gain_rate(self, cell: Cell, j: int, t: float) -> float:
        ...
        # Drug modulation
        u = self.get_drug_conc(t).get("ecdna_destabilizer", 0.0)
        if u > 0:
            base_rate *= drug_effect(u, cfg.DRUGS["ecdna_destabilizer"], "inhibit")
        
        return base_rate
    
    def ecdna_loss_rate(self, cell: Cell, j: int, t: float) -> float:
    ...
    def all_ecdna_rates(self, cell: Cell, t: float) -> List[Tuple[str, dict, float]]:
        """Get all ecDNA gain/loss rates."""
        rates = []
        for j in range(cfg.J_ECDNA):
            # Gain
            r_gain = self.ecdna_gain_rate(cell, j, t)
            if r_gain > 0 and cell.k[j] < cfg.K_MAX[j]:
                rates.append(("ecdna_gain", {"j": j}, r_gain))
            
            # Loss
            r_loss = self.ecdna_loss_rate(cell, j, t)
            if r_loss > 0:
    ...
    def division_hazard(self, cell: Cell, t: float) -> float:
        """
        ...
        k_effect = 1.0 + cfg.ECDNA_FITNESS_EFFECT * cell.total_ecdna()
    ...
    def death_hazard(self, cell: Cell, t: float) -> float:
        """
        ...
        drug_mod = 1.0
        u = self.get_drug_conc(t).get("senolytic", 0.0)
    ...
    def compute_all_rates(self, cell: Cell, t: float) -> Tuple[List[Tuple], float]:
        ...
        # Switching rates
        channels.extend(self.all_switch_rates(cell, t))
        
        # ecDNA rates
        channels.extend(self.all_ecdna_rates(cell, t))
        
        # Terminal events
        div_rate = self.division_hazard(cell, t)
        if div_rate > 0:
            channels.append(("division", {}, div_rate))
        
        death_rate = self.death_hazard(cell, t)
        if death_rate > 0:
